...
## [2026-05-01] Geo: адресные координаты ошибочно могли стать центром города

**Симптом:** при выборе адреса из DaData формы передавали `geo_lat/geo_lon` в `ensureCityAction`, а тот записывал эти координаты в `City.lat/lng`. Для полного адреса это координаты дома, а не центра города/населенного пункта.

**Причина:** слой создания города смешивал два разных типа координат: city center и order/listing address point.

**Решение:**
- `ensureCityAction` теперь валидирует вход через Zod и использует `lat/lng` только при явном `isCityCenter: true`.
- Формы заказов и объявлений больше не передают координаты адреса в `ensureCityAction`.
- Координаты заказа сохраняются отдельно в `Order.lat/lng` и синхронизируются с `orderLocation` через PostGIS.

**Security verdict:** PASS_WITH_NOTES — снижена вероятность неверной геофильтрации и утечки точного адреса через городские координаты.
...
## [2026-05-01] UX/UI: Мерцание балунов и задержка открытия на Yandex Maps

**Симптом:** При клике на метку на общей карте заказов (`YandexOrdersMap`), балун открывался с заметной задержкой, а вся карта "мигала" (метки исчезали и появлялись заново).

**Причина:** Реактивное управление состоянием `selectedPointId` вызывало срабатывание `useEffect` с методом `updateFeatures`. Внутри этого метода выполнялся `clusterer.removeAll()`, что полностью очищало карту и заново отрисовывало все метки при каждом открытии/закрытии балуна.

**Решение:**
- Переход на **императивное управление** стилями через API Яндекс.Карт.
- Удален `selectedPointId` из зависимостей `updateFeatures`, чтобы избежать полной перерисовки слоя.
- Изменение цвета метки (preset) теперь происходит мгновенно внутри обработчиков событий `balloonopen` и `balloonclose` через `placemark.options.set('preset', ...)`.
- **Добавлена блокировка `boundschange`** при открытом балуне. Это предотвращает ситуацию, когда авто-панорамирование карты для балуна вызывало обновление вьюпорта, рефетч данных и очистку карты через `clusterer.removeAll()`, что приводило к закрытию балуна.
- **Внедрена проверка `setPoints` на глубокое равенство** (по ID). Если данные не изменились после смены вьюпорта, перерисовка слоя не происходит.
- Расширены типы `YMaps2Instance` и `YMaps2Object` в `shared/lib/yandex-maps.ts` для поддержки методов `options.set` и управления курсорами (`cursors.push`).

**Security verdict:** PASS — изменение касается только клиентского UI/UX и типизации внешнего API.
