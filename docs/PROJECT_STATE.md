...
- **Geo-Location UX**: В `CitySelector` исправлен агрессивный редирект. Теперь при выборе города на главной странице (/) выполняется просто `reload()`, что позволяет оставаться на лендинге. В ленте заказов реализована умная замена сегмента города в URL.
...
- **Verification**: `tsc` — PASS, ручная проверка редиректов на главной и в ленте.
