...
> 🕓 Последнее обновление: 2026-05-03 (Routing, Auth & Geo UX)

---

## Обновление 2026-05-03 (Routing, Auth & Geo UX)
- **Universal Order Links**: `order.service.ts` переведен на использование `/orders/v/[slug]` в качестве основного формата ссылок (включая ссылки с карты). Это гарантирует стабильную работу переходов при смене категорий или городов заказа.
- **Auth UX**: Неавторизованные пользователи при попытке доступа к деталям заказа теперь перенаправляются на `/auth/login` с сохранением `callbackUrl`, вместо сброса на главную страницу.
- **Geo-Location UX**: В `CitySelector` добавлено тихое автоопределение города при первом посещении. Если город не выбран, система пытается определить его по IP/Геолокации и сохранить в куки без принудительной перезагрузки страницы.
- **Map DoS Protection**: Лимиты BBox в API поиска по карте расширены до 30°x60°, что позволяет корректно отображать крупные федеральные округа и регионы России без срабатывания защиты от сканирования.
- **CSP Optimization**: Удален `log.api-maps.yandex.ru` из `connect-src` для предотвращения лишних аналитических запросов и очистки консоли браузера от ошибок блокировки.
- **Verification**: `tsc` — PASS (за исключением внешних .next типов), `eslint` — PASS (в затронутых файлах), ручная проверка редиректов и автоопределения города.
...
## Обновление 2026-05-01 (Yandex Maps Stabilization & Cleanup)
- **Map Interaction Perfected**: Устранено мерцание карты при открытии балунов за счет перехода на императивное управление стилями. Исправлен баг самопроизвольного закрытия балуна при авто-панорамировании через блокировку `boundschange`.
- **TypeScript & IDE Hygiene**: Исправлены ошибки типизации (missing `balloon`, `getBounds`). Папки `scratch` и `test-results` исключены из проверки TypeScript (`tsconfig.json`), что убрало визуальный шум в IDE.
- **Repository Cleanup**: Удален технический файл `.last-run.json` из индекса git. Настроено корректное игнорирование временных файлов.
- **Verification**: `tsc` — PASS (за исключением внешних scratch-скриптов, которые теперь игнорируются).
