...
- **Geo-Location UX**: Исправлен баг в `CitySelector` — добавлены `type="button"` ко всем кнопкам модалки, что предотвращает сабмит формы поиска в `HeroSection`. На главной выбор теперь происходит без перезагрузки.
- **SEO & Search URL**: `HeroSection` теперь строит slug-based URL (`/orders/moskva?search=...`). В `CitySelector` добавлено сохранение `citySlug` в куки.
- **Map DoS Protection**: Лимиты BBox расширены до 30°x60° для корректного отображения крупных регионов.
- **CSP Optimization**: Удален `log.api-maps.yandex.ru` из `connect-src` для чистоты консоли.
- **Verification**: `tsc` — PASS, ручная проверка UX и Routing.
