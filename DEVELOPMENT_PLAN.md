...
| 10 | Geo-поиск и мульти-город (10.1–10.2 geo, 10.3–10.6 карты) | 🟡 Частично реализована | 70% |
...
| 10. Geo/мульти-город | 🟡 | Web-geo, PostGIS radius, Yandex inline/order map and clustered map implemented; AddressPicker remains. |
...
> **Статус: 🟡 Частично реализована (2026-05-01): web-geo, Yandex Maps, inline-карта заказа и карта заказов с кластерами готовы; 10.6 AddressPicker остается отдельным шагом.**
...
10.3.2  Геокодирование адреса при создании заказа
        - Геокодер: DaData Suggest для выбранного адреса + Яндекс Geocoder API fallback
        - Ключ Яндекс Геокодера: `YANDEX_GEOCODER_API_KEY`, получить на developer.tech.yandex.ru
...
10.4.1  Технологический стек (веб):
        - Библиотека: Yandex Maps JS API v3 через глобальный loader `ymaps3`
        - Тайлы и маркеры: Яндекс Карты, ключ `NEXT_PUBLIC_YANDEX_MAPS_API_KEY`
        - Компонент только клиентский ('use client'), API загружается после `ymaps3.ready`
        
10.4.2  Компонент: shared/ui/YandexOrderMap.tsx
        Входные данные:
        ...
        - Интерактивность отключена (`behaviors: []`)
          — чтобы не мешала скроллу страницы
        - Внешняя ссылка на карту убрана; адрес виден прямо под встроенной картой
        - Если lat/lng = null — показывать текстовый адрес как раньше

10.4.3  Интеграция в страницу заказа:
        - app/(main)/orders/[citySlug]/[slug]/[orderSlug]/page.tsx
        - Заменить внешнюю ссылку адреса на <YandexOrderMap lat={order.lat} lng={order.lng} address={order.address} />
        - Карта рендерится только если order.lat != null

10.4.4  Маркер:
        - Кастомная иконка в стиле дизайна (цвет primary из темы)
        - DOM-маркер Yandex Maps JS API v3: круг с тенью, совпадающий по цвету с --color-primary
```
...
```
10.5.1  Зависимости:
        - npm-зависимости не добавляются
        - кластеризация через `ymaps3.import("@yandex/ymaps3-clusterer")`
        - TypeScript-типы описаны локально в shared/lib/yandex-maps.ts
...
10.5.3  Компонент: widgets/OrdersMap/ui/YandexOrdersMap.tsx ('use client')
        - Yandex Maps JS API v3 + YMapClusterer
...
10.5.5  Зоны поиска (опционально, Фаза 2):
        - Рисование зоны на карте через отдельный инструмент поверх Yandex Maps API
...
10.6.1  Компонент: shared/ui/AddressPicker.tsx ('use client')
        - Поле ввода адреса с автокомплитом (DaData Suggest или Яндекс Suggest API)
...
```
Технология:     Yandex Maps JS API v3 (`api-maps.yandex.ru/v3`) + YMapClusterer
                без Leaflet и OpenStreetMap; нужен публичный referer-restricted JS API key
Геокодирование: DaData Suggest как основной источник координат выбранного адреса
                + Yandex Geocoder API fallback через `YANDEX_GEOCODER_API_KEY`
Координаты:     lat Float? + lng Float? на модели Order (простота)
                orderLocation PostGIS — оставить для geo-поиска в 10.2
Маршруты:       Внешняя ссылка убрана из заказа; адрес показывается внутри inline-карты
Кластеры:       `@yandex/ymaps3-clusterer` через runtime import
SSR:            Карты только в client components после `ymaps3.ready`
```
